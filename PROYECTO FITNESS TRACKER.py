while True:
    ...
    if elección == 1:
        ...
        if resultado_IMC < 18.5 :
            ...
            print ('Tu IMC es:', resultado_IMC,'. Tú IMC está en el rango saludable. ¡Buen trabajo!')
            print("\n" + "-"*40 + "\n")
        elif 25 <= resultado_IMC <= 29.9  :
            print ('Tu IMC es:', resultado_IMC,'. Tú IMC indica sobrepeso. Estás por encima del rango saludable, sería recomendable cuidar la alimentación y la actividad física.')
            print("\n" + "-"*40 + "\n")
        elif 30 <= resultado_IMC <= 34.9  :
            print ('Tu IMC es:', resultado_IMC,'. Tú IMC indica obesidad grado I. Estás en un nivel de obesidad inicial, lo recomendable es trabajar en dieta y ejercicio constantes')
            print("\n" + "-"*40 + "\n")
            ...
            print ('Tú IMC es:', resultado_IMC,'. Tú IMC indica obesidad grado III (mórbida). Este nivel implica un riesgo serio para la salud, es muy importante buscar atención médica especializada.')
            print("\n" + "-"*40 + "\n")
    elif elección == 2:
        edad = int(input('Introduce tu edad:'))
        sexo = input('Introduce tu sexo (M/F):')
        peso = float(input('Introduce tu peso (kg):'))
        altura = float(input('Introduce tu altura (cm):'))
        nivel_actividad = input('Introduce tu nivel de actividad (sedentario, activo, muy activo):').lower()
        if sexo == 'M':
        ...
        objetivo = input('Introduce tu objetivo (''bajar peso, mantener peso, subir peso):').lower()
